...
Person personJerry = fans.FirstOrDefault(x => x.FirstName.ToLower() == "Jerry".ToLower()); //we can also use Where(x => x.FirstName.ToLower() == "Jerry".ToLower()).FirstOrDefault()
if(personJerry == null)
{
...
{
    personJerry.FavouriteSongs = songs.Where(x => x.Title.ToUpper().StartsWith("B")).ToList();
...
if(personJane != null) //here we need the case where we found Jane
{
...
//Select all fans name from the fans array that have 4 or more songs.
//WE ALWAYS NEED TO DO THE CONDITION CHECK BEFORE WE SELECT SOME PROPERTY (AFTER THE SELECT WE WON'T HAVE ACCESS TO THE PROPERTY)
